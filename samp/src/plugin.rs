...
// ---------------------------------------------------------------------------
// Pawn include generation
// ---------------------------------------------------------------------------

/// Pawn `.inc` source for every native this plugin registers.
///
/// The declarations are derived by `#[native]` from the Rust signatures, so the
/// include cannot drift from the code: rename an argument or change a type and
/// the next build produces the new line.
///
/// Types map as follows — `f32` to `Float:`, `bool` to `bool:`, `AmxString` to
/// `const name[]`, `Ref<T>` to `&name` keeping `T`'s tag, and buffers to
/// `name[]`. A type the macro does not recognize becomes a plain cell, which is
/// what the AMX passes anyway; only the Pawn tag is lost. Natives declared
/// `raw` come back commented out, since their arity is not in the signature.
///
/// Returns an empty include (header only) before the plugin is loaded.
#[must_use]
...
fn render_pawn_include(name: &str, decls: &[&str]) -> String {
    ...
    let guard: String = name
        ...
        .collect();
    let mut out = format!(
        "// Generated by rust-samp from the #[native] signatures of {name}.\n\
         // Edits are lost on the next build — change the Rust side instead.\n\n\
         #if defined _{guard}_included\n    #endinput\n#endif\n#define _{guard}_included\n\n"
    );
    for decl in decls {
        out.push_str(decl);
        out.push('\n');
...
/// Writes [`pawn_include`] to `path`, creating or truncating the file.
///
/// # Errors
/// Returns the underlying [`std::io::Error`] when the path cannot be written.
pub fn write_pawn_include(path: impl AsRef<std::path::Path>) -> std::io::Result<()> {
    std::fs::write(path, pawn_include())
}

/// Writes the include when `SAMP_PAWN_INCLUDE` holds a path.
///
/// Called by the SDK once the natives are known, so a plugin author gets the
/// `.inc` by starting the server once with the variable set, on either server
...
pub(crate) fn write_pawn_include_if_requested() {
    ...
    match write_pawn_include(&path) {
        ...
        Err(e) => crate::macros::sdk_warn!(
            "could not write the Pawn include to {}: {e}",
